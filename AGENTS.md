...
반드시 질문하거나 팀 합의가 필요한 항목:

- `API_CONTRACT.md`, `DB_SCHEMA.md`, `ROUTE_OWNERSHIP.md`에 없는 API, 컬럼, 상태값, enum을 새로 추가해야 하는 경우
- 담당자가 다른 route/API/DB table에 쓰기 로직을 추가해야 하는 경우
- 공통 error/auth/pagination 정책을 변경해야 하는 경우
- `docs/openapi.yaml`과 사람 기준 계약 문서가 충돌하는 경우
- 공개 API, 로그인 필요 API, 관리자 API의 권한 경계를 바꿔야 하는 경우
