...
```powershell
Invoke-RestMethod -Method Post "http://localhost:8080/api/admin/parts/external-offers/refresh?category=GPU&limit=20"
```

내부 자산 후보 대량 수집:

```powershell
Invoke-RestMethod -Method Post "http://localhost:8080/api/admin/parts/catalog/refresh?category=GPU&limitPerQuery=3"
```

후보를 바로 내부 자산으로 게시하면서 수집:

```powershell
Invoke-RestMethod -Method Post "http://localhost:8080/api/admin/parts/catalog/refresh?category=GPU&limitPerQuery=3&publish=true"
```
